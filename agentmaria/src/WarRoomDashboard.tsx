...
export default function WarRoomDashboard() {
  const [status, setStatus] = useState<'running' | 'paused' | 'complete'>('running');
  const [tick, setTick] = useState(0);
  const [agents, setAgents] = useState<Record<string, Agent>>(JSON.parse(JSON.stringify(INITIAL_AGENTS)));
  ...
  const addFeedEvent = (msg: string, colorStr: string) => {
    setFeed(prev => {
  ...
  const calcProgress = (startTick: number, durationTicks: number, currentTick: number) => {
  ...
  const handleViewFullOutput = (e: any, agentName: string) => {
    e.stopPropagation();
    // Stub definition as requested
    const promptFunc = (window as any).sendPrompt || alert;
  ...
  const avgConf = confAgents.length > 0 
    ? (confAgents.reduce((acc, curr) => acc + (curr.conf || 0), 0) / confAgents.length).toFixed(2)
    : "0.00";
